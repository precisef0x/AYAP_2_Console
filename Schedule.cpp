...
Schedule &Schedule::operator+(const Schedule &input) {
  ...
  if (open.hours * 60 + open.minutes >=
      input.open.hours * 60 + input.open.minutes)
    newOpen = input.open;
  else
    newOpen = open;
  if (close.hours * 60 + close.minutes <=
      input.close.hours * 60 + input.close.minutes)
    newClose = input.close;
  else
    newClose = close;
...
string Schedule::printOpen() {
  stringstream buffer;
  buffer << ((open.hours > 9) ? "" : "0") << open.hours << ':'
         << ((open.minutes > 9) ? "" : "0") << open.minutes;
  return buffer.str();
}

string Schedule::printClose() {
  stringstream buffer;
  buffer << ((close.hours > 9) ? "" : "0") << close.hours << ':'
         << ((close.minutes > 9) ? "" : "0") << close.minutes;
  return buffer.str();
}

Time Schedule::getOpenTime() const { return open; }

Time Schedule::getCloseTime() const { return close; }
